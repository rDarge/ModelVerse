...
const ChatMessage: FC<ChatMessageProps> = ({ sender, text, avatarUrl, imageUrl }) => {
  ...
  const messageBubbleClasses = cn(
    ...
    isSystem ? 'bg-muted text-muted-foreground text-sm italic text-center w-full max-w-full py-2' : 'relative group' // Add relative and group for non-system messages
  );
